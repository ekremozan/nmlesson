...
| User identity in crash reports | `FirebaseCrashlytics.setUserId(...)` is never called; the SDK's own anonymous installation id is the only identifier attached | The project has no real auth/account system (`MockEntitlementRepository` is in-memory), so there is no real user id to attach, and adding one would be a privacy liability for no debugging benefit | Once real accounts exist, a considered decision (not a default) about what identifier — if any — is safe to attach |
