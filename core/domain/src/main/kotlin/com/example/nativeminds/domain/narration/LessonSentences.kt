 ...
 * that decides what a [LessonNarrator] speaks also tells the reader which characters to draw the
...
/**
 * Splits lesson paragraphs into sentences, keeping each one's position in its paragraph.
 *
 ...
 */
fun List<String>.toLessonSentences(): List<LessonSentence> =
    flatMapIndexed { paragraphIndex, paragraph -> paragraph.sentencesAt(paragraphIndex) }

/** The spoken form of [toLessonSentences] — the text a narrator hands to a speech engine. */
fun List<String>.sentenceTexts(): List<String> =
    toLessonSentences().map { this[it.paragraphIndex].substring(it.start, it.end) }

/**
 * Running word totals across [toLessonSentences], one entry per sentence boundary.
 *
 * `this[i]` is how many words come before sentence `i` and the last entry is the lesson's own word
 * count, which is what turns "speaking word 250 of sentence 12" into a fraction of the whole lesson
...
/** Words in a stretch of lesson text — whitespace-separated runs, the unit narration is paced in. */
fun String.wordCount(): Int = WORD.findAll(this).count()

private val WORD = Regex("\\S+")

private fun String.sentencesAt(paragraphIndex: Int): List<LessonSentence> {
...
private fun MutableList<LessonSentence>.addTrimmed(
    paragraph: String,
    paragraphIndex: Int,
    from: Int,
    to: Int,
    ...
    if (start < end) add(LessonSentence(paragraphIndex, start, end))
}
